...
## Category
SQL Injection - UNION-based (Database Version Enumeration on Oracle)

## Vulnerability Summary
The website's product filtering feature contains a SQL injection vulnerability that allows attackers to retrieve database version information. By using a UNION-based SQL injection payload in the category parameter, the application exposes the Oracle database version and banner information. This vulnerability enables attackers to fingerprint the database system, which is often the first step in planning more advanced attacks.
...
4. **Payload Construction:** Crafted a UNION-based SQL injection payload to query the v$version table.
5. **Execution:** The injected payload retrieves the Oracle database banner and version information.
6. **Verification:** Confirmed successful data extraction by observing the database version displayed in the response.

![Lab 03 Screenshot 1](screenshot1.png)
...
- **Oracle-Specific Tables:** Oracle's v$version table contains database version information accessible through SQL.
...
- The injection transforms it to: `SELECT * FROM products WHERE category = 'gifts' UNION SELECT banner, NULL FROM v$version--'`
- The `'` closes the category string value.
- The `UNION SELECT` combines the original query results with data from v$version.
- The `banner` column contains the Oracle database version string.
...
5. **Database Permissions:** Restrict access to system tables like v$version for application database accounts.
